...
- `src/client/public/docs/cyberia/` — Cyberia architecture, server, client, CLI, action, quest, economy and roadmap.
- `src/client/public/docs/object-layer/` — Object Layer protocol, identity model, lifecycle and runtime semantics. Cyberia consumes them; it defines none.
- `src/client/public/docs/item-ledger/` — on-chain registration, ownership and the `ObjectLayerToken` contract.

## Documentation

All authored documentation lives in one tree, `src/client/public/docs/`. The path is the identity:
`<domain>/<category>/<slug>.md`.

- Domains: `ecosystem`, `object-layer`, `item-ledger`, `cyberia`, `cryptokoyn`, `nexodev`, `underpost`.
- Categories: `overview`, `explanation`, `how-to`, `reference`. `underpost` uses `engineering-journal`, `lab-notes`, `adr`. `ecosystem` uses `architecture`.
- One topic per file. One `#` title per file. Lowercase, hyphenated filenames.
- A domain declares itself once, in the front matter of its overview (`domain`, `identity`, `maturity`, `order`).
- Link to another document by its relative path (`../reference/cyberia-cli.md#anchor`). Link to source by code span, never by link.
- Navigation is generated from the tree. Do not add "related documents" lists.
- `npx vitest run --project ecosystem:contract test/ecosystem/contract/documentation-architecture.test.js` is the quality gate: broken links, unknown categories, duplicate identities and titles fail it.
- `nexodev/reference/underpost-cli.md` is generated by `node bin/deploy cli-docs`. Never hand-edit it.
