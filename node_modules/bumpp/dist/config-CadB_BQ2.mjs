...
function getNextVersion(currentVersion, bump, commits) {
	...
	else if (bump.type === "conventional") type = determineSemverChange(commits);
	else if (bump.type === "conventional-prerelease") type = oldSemVer.prerelease?.length ? "prerelease" : `pre${determineSemverChange(commits)}`;
	else type = bump.type;
...
function getNextVersions(currentVersion, preid, commits) {
	const next = {};
	const currentPreid = parse(currentVersion).prerelease?.[0];
	if (typeof currentPreid === "string") preid = currentPreid || "preid";
...
async function promptForNewVersion(operation, commits) {
	...
	const answers = await (0, import_prompts.default)([{
		...
		choices: [
			...
			{
				value: "conventional-prerelease",
				title: `${"conventional".padStart(PADDING, " ")} ${styleText("bold", next["conventional-prerelease"])}`
			},
			...configCustomVersion ? [{
				value: "config",
...
*/
function hasNamedTokens(template) {
...
function renderTemplate(template, tokens) {
	if (hasNamedTokens(template)) return template.replace(new RegExp(TOKEN_CAPTURE_SOURCE, "g"), (_, key) => tokens[key] ?? "");
	if (template.includes("%s")) return template.replaceAll("%s", tokens.version);
...
*/
function buildTokens(operation) {
	...
	const tagName = operation.options.tag ? renderTemplate(operation.options.tag.name, base) : `v${newVersion}`;
	return {
		...base,
		tag: tagName
	};
...
async function getDefaultBranch() {
	...
	if (symbolic.exitCode === 0 && symbolic.stdout) return symbolic.stdout.replace(/^refs\/remotes\/origin\//, "");
	const abbrev = await git([
		"rev-parse",
		"--abbrev-ref",
		"origin/HEAD"
	]);
	if (abbrev.exitCode === 0 && abbrev.stdout) return abbrev.stdout.replace(/^origin\//, "");
	return "main";
}
/**
* Fetches the given branch from `origin` (best effort).
*/
async function gitFetch(branch) {
	await git([
		"fetch",
		"origin",
		branch
...
*/
async function isBehindRemote(branch) {
...
*/
async function localBranchExists(name) {
...
*/
async function checkoutNewBranch(name, force = false) {
	await x("git", [
		"checkout",
		force ? "-B" : "-b",
		name
...
*/
async function pushBranch(name, force = false) {
	const args = [
		"push",
		"--set-upstream",
		"origin",
...
async function getGithubRepo() {
	...
	if (exitCode !== 0 || !stdout) return void 0;
...
async function normalizeOptions(raw) {
	...
	else if (isReleaseType(raw.release)) release = {
		type: raw.release,
		preid
	};
	else release = {
	...
	let pr;
	if (raw.pr) {
		if (!push) throw new Error("The `pr` option requires `push` to be enabled (remove `--no-push`).");
		if (tag) {
		...
		pr = {
			branch: rawPr.branch || "release/v{version}",
			base: rawPr.base,
			title: rawPr.title,
			body: rawPr.body,
...
*/
async function checkPrPreconditions(operation, interactive) {
	const pr = operation.options.pr;
	if (!await isWorkingTreeClean()) throw new Error("Git working tree is not clean. Commit or stash your changes before releasing.");
	...
	if (originalBranch !== baseBranch) {
		...
		if (!interactive) throw new Error(`${message} Switch to "${baseBranch}" or set \`pr.base\`.`);
		console.log(symbols.info, message);
		if (!await confirm("Continue anyway?", false)) throw new Error("Aborted.");
	}
	await gitFetch(baseBranch);
	if (await isBehindRemote(baseBranch)) {
		const message = `Local "${baseBranch}" is behind "origin/${baseBranch}".`;
		if (!interactive) throw new Error(`${message} Pull the latest changes before releasing.`);
		console.log(symbols.info, message);
		if (!await confirm("Continue anyway?", false)) throw new Error("Aborted.");
	}
	return {
...
*/
async function startPrBranch(operation, ctx, interactive) {
	const pr = operation.options.pr;
	const tokens = buildTokens(operation);
	ctx.branchName = renderTemplate(pr.branch, tokens);
	const [existsLocal, existsRemote] = await Promise.all([localBranchExists(ctx.branchName), remoteBranchExists(ctx.branchName)]);
	...
	if (ctx.existingPr) {
		...
		if (!interactive) throw new Error(`${message} Delete it or re-run without --yes to recreate it.`);
		console.log(symbols.info, message);
		if (!await confirm("Recreate it and force-push?", false)) throw new Error("Aborted.");
		ctx.forcePush = true;
	}
...
*/
async function finishPrRelease(operation, ctx, commits, interactive) {
	const pr = operation.options.pr;
	const tokens = buildTokens(operation);
	...
	if (!await isGhAvailable() || !repo) {
		printManualInstructions(repo, ctx, title, body, pr.draft);
		return;
	}
	if (ctx.existingPr) {
		if (!(interactive ? await confirm(`Update pull request #${ctx.existingPr.number} into "${ctx.baseBranch}"?`) : true)) {
		...
		const result = await x("gh", [
			"pr",
			"edit",
			String(ctx.existingPr.number),
			"--title",
			title,
			"--body",
			body
		], { throwOnError: false });
		if (result.exitCode === 0) console.log(symbols.success, `Pull request updated: ${styleText(["cyan", "bold"], ctx.existingPr.url)}`);
		else {
			console.log(styleText("yellow", `Failed to update pull request #${ctx.existingPr.number} via \`gh\`.`));
			if (result.stderr.trim()) console.log(styleText("gray", result.stderr.trim()));
		}
		return;
	}
	if (!(interactive ? await confirm(`Create pull request from "${ctx.branchName}" into "${ctx.baseBranch}"?`) : true)) {
		printManualInstructions(repo, ctx, title, body, pr.draft);
		return;
	}
	const args = [
...
*/
async function cleanupPrBranch(ctx) {
	try {
		if (await getCurrentBranch() !== ctx.originalBranch) await checkoutBranch(ctx.originalBranch);
		if (ctx.branchName) await deleteLocalBranch(ctx.branchName);
	} catch {}
}
function resolveBody(pr, tokens, commits) {
	if (typeof pr.body === "function") return pr.body(tokens);
	if (typeof pr.body === "string") return renderTemplate(pr.body, tokens);
...
function groupCommits(commits) {
	...
	if (breaking.length) sections.push({
...
function formatCommitLine(commit) {
	...
	const prRefs = commit.references.filter((ref) => ref.type === "pull-request").map((ref) => ref.value);
	const refs = prRefs.length ? ` (${prRefs.join(", ")})` : "";
	return `- ${scope}${commit.description}${refs} (${commit.shortHash})`;
}
function defaultPrBody(tokens, commits) {
	const lines = [`Release \`${tokens.tag}\` (\`${tokens.oldVersion}\` → \`${tokens.version}\`).`];
	for (const section of groupCommits(commits)) {
		lines.push("", `### ${section.title}`, "");
		for (const commit of section.commits) lines.push(formatCommitLine(commit));
	}
	return lines.join("\n");
}
/**
...
async function isGhAvailable() {
	try {
		if ((await x("gh", ["--version"], { throwOnError: false })).exitCode !== 0) return false;
		return (await x("gh", ["auth", "status"], { throwOnError: false })).exitCode === 0;
	} catch {
		return false;
	}
}
function printManualInstructions(repo, ctx, title, body, draft) {
	console.log();
	...
	} else console.log("  Open a pull request from your release branch on your Git host.");
	console.log(`  ${styleText("gray", "branch")} ${ctx.branchName} ${styleText("gray", "→")} ${ctx.baseBranch}`);
	console.log(`  ${styleText("gray", "title ")} ${title}`);
	if (draft) console.log(`  ${styleText("gray", "draft ")} yes`);
	console.log();
...
async function versionBump(arg = {}) {
	...
	let prCtx;
	if (operation.options.pr) prCtx = await checkPrPreconditions(operation, interactive);
	if (arg.confirm) {
	...
	try {
		if (operation.options.pr && prCtx) await startPrBranch(operation, prCtx, interactive);
		await runRelease(operation, commits, prCtx, interactive);
	} catch (error) {
		if (operation.options.pr && prCtx) await cleanupPrBranch(prCtx);
		throw error;
	}
	return operation.results;
}
async function runRelease(operation, commits, prCtx, interactive) {
	...
	if (operation.options.pr && prCtx) await finishPrRelease(operation, prCtx, commits, interactive);
	else await gitPush(operation);
}
function printSummary(operation) {
	const tokens = buildTokens(operation);
	console.log();
	console.log(`   files ${operation.options.files.map((i) => styleText("bold", i)).join("\n         ")}`);
	if (operation.options.commit) console.log(`  commit ${styleText("bold", renderTemplate(operation.options.commit.message, tokens))}`);
	if (operation.options.tag) console.log(`     tag ${styleText("bold", renderTemplate(operation.options.tag.name, tokens))}`);
	if (operation.options.pr) console.log(`      pr ${styleText("bold", renderTemplate(operation.options.pr.branch, tokens))} ${styleText("gray", "→ pull request")}`);
