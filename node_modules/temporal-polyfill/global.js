!function() {
  ...
  function computeCalendarMonthCodeParts(calendar, year, month) {
    return calendar ? calendar.O(year, month) : (month => [ month, 0 ])(month);
  }
  ...
  function computeCalendarIsoFieldsFromParts(calendar, year, month, day) {
    return calendar ? calendar.je(year, month, day) : ((year, month, day) => ({
      year: year,
      month: month,
      day: day
    }))(year, month, day);
  ...
  function computeCalendarDaysInYear(calendar, isoDate) {
    const {year: year} = computeCalendarDateFields(calendar, isoDate);
    return calendar ? calendar.j(year) : (year => computeIsoInLeapYear(year) ? 366 : 365)(year);
  }
  ...
  function diffCalendarDates(calendar, startIsoDate, endIsoDate, largestUnit) {
    ...
    if (8 === largestUnit) {
      ...
      let days = 0;
      if (sign) {
        months = calendar ? calendar.ne(year0, month0, year1, month1) : ((year0, month0, year1, month1) => 12 * (year1 - year0) + month1 - month0)(year0, month0, year1, month1);
  ...
  function resolveCalendarYear(calendar, fields) {
    ...
    void 0 !== era || void 0 !== eraYear) {
      ...
      const yearByEra = exoticCalendar?.te ? exoticCalendar.te(eraYear, normalizedEra, eraOrigin) : ((eraYear, eraOrigin) => (eraOrigin + eraYear) * (Math.sign(eraOrigin) || 1) || 0)(eraYear, eraOrigin);
