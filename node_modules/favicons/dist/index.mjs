...
async function sourceImages(src) {
	...
	else if (typeof src === "string") return await sourceImages(await readFile(src));
...
async function resize(source, width, height, pixelArt) {
	...
	} else return await sharp(source.data).ensureAlpha().resize({
		width,
		height,
		fit: sharp.fit.contain,
		background: "#00000000",
...
var WindowsPlatform = class extends Platform {
	...
	browserConfig() {
		...
		const contents = new xml2js.Builder({ xmldec: {
			version: "1.0",
			encoding: "utf-8",
			standalone: void 0
...
var FaviconStream = class extends Transform {
	#options;
	#handleHTML;
