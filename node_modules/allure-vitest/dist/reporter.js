...
import { ReporterRuntime, createDefaultWriter, getEnvironmentLabels, getFallbackTestCaseIdLabel, getFrameworkLabel, getHostLabel, getLanguageLabel, getPackageLabel, getSuiteLabels, getThreadLabel, md5, } from "allure-js-commons/sdk/reporter";
...
const normalizeSetupFilePath = (setupFilePath) => setupFilePath.startsWith("file://") ? fileURLToPath(setupFilePath) : setupFilePath;
export default class AllureVitestReporter {
    ...
    registerSetupFile(vitest) {
        var _a, _b;
        for (const project of vitest.projects) {
            const setupFilePath = project.config.browser.enabled ? browserSetupModulePath : setupModulePath;
            const hasSetupFile = project.config.setupFiles.some((setupFile) => normalizeSetupFilePath(setupFile) === setupFilePath);
            if (!hasSetupFile) {
                project.config.setupFiles.unshift(setupFilePath);
            }
            if (project.config.browser.enabled) {
                (_a = project.config.browser).commands ?? (_a.commands = {});
                for (const [name, command] of Object.entries(allureBrowserCommands)) {
                    (_b = project.config.browser.commands)[name] ?? (_b[name] = command);
                }
            }
        }
    }
    enableConcurrencySupport(vitest) {
        for (const project of vitest.projects) {
            if (!project.config.browser.enabled) {
    ...
    handleTask(task) {
        ...
        const { allureRuntimeMessages = [], allureGlobalRuntimeMessages = [], vitestWorker, browser, allureSkip = false, } = task.meta;
        ...
        const { projectName, specPath, fullName, legacyFullName, name, suitePath, labels: metadataLabels, links: metadataLinks, } = getTestMetadata(task);
        ...
        this.allureReporterRuntime.updateTest(testUuid, (result) => {
            ...
            const runtimeMessages = (this.config.reportMatchers ?? true)
