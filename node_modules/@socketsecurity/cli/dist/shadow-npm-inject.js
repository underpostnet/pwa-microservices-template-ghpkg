...
function getUrlOrigin(input) {
  ...
  return url.parseUrl(input)?.origin ?? '';
}
async function getAlertsMapFromArborist(arb, needInfoOn, options) {
  const opts = {
    ...
    nothrow: false,
    ...options,
    filter: utils.toFilterConfig(objects.getOwn(options, 'filter'))
  };
...
const require$1 = require$$1.createRequire((typeof document === 'undefined' ? require('u' + 'rl').pathToFileURL(__filename).href : (_documentCurrentScript && _documentCurrentScript.tagName.toUpperCase() === 'SCRIPT' && _documentCurrentScript.src || new URL('shadow-npm-inject.js', document.baseURI).href)));
...
installSafeArborist();
//# debugId=ebce0dd1-3c0e-4c35-a9ed-03b774d73b22
