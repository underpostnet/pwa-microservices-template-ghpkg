...
async function extractPackagePurlsFromPackageJson(cwd = process.cwd()) {
  ...
  try {
    ...
    debug.debugFn('warn', `${constants.PACKAGE_JSON} not found or invalid during dependency scanning`);
...
async function shadowPnpmBin(args = process.argv.slice(2), options, extra) {
        ...
        try {
          ...
          if (lockfileContent) {
            ...
            if (lockfile) {
              ...
              debug.debugFn('notice', `scanning: all dependencies from ${constants.PNPM_LOCK_YAML}`);
              ...
              debug.debugFn('notice', 'complete: lockfile scanning, proceeding with install');
            }
          }
        } catch (e) {
          debug.debugFn('error', `${constants.PNPM} lockfile scanning failed`);
          debug.debugDir('error', e);
        }
      } else {
        debug.debugFn('notice', `skip: no ${constants.PNPM_LOCK_YAML} found, skipping bulk install scanning`);
      }
    }
    debug.debugFn('notice', 'complete: scanning, proceeding with install');
  }
  ...
  debug.debugFn('notice', `spawn: ${constants.PNPM} shadow bin ${realPnpmPath} ${utils.cmdFlagsToString(suffixArgs)}`);
  if (wasSpinning) {
    spinner?.start();
  }
...
//# debugId=b17ded58-7fff-48da-9275-781b50922d97
